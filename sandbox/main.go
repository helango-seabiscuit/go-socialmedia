...
func main() {
	...
	updatedUser, err := c.UpdateUser("test@example.com", "password", "john", "20")
	if err != nil {
		log.Fatal(err)
	}
	...
	post, err := c.CreatePost("test@example.com", "my dog is way too cute")
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println("post creeated", post)
